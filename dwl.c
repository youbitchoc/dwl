...
static struct wlr_relative_pointer_manager_v1 *relative_pointer_manager;
...
buttonpress(struct wl_listener *listener, void *data)
{
	...
	case WLR_BUTTON_PRESSED:
		...
		if (cursor_mode == CurResize) break;
...
moveresize(const Arg *arg)
{
	...
	wlr_cursor_set_xcursor(cursor, cursor_mgr, "target");
}
...
zoom(const Arg *arg)
{
	...
	/* Search for the first tiled window */
	wl_list_for_each(c, &clients, link)
	...
	if (c == sel) {
		/* sel is first tiled window
		 * set sel to last tiled window */
